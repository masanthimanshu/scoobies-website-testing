...
class RootCauseAnalyst {
   ...
   * @param {Object} params.domSnapshot - Key elements extracted from DOM
   * @param {Array} params.networkData - List of anomalous/failed requests and summary
   * @param {Array} params.consoleErrors - List of console errors/warnings
   * @param {Object} params.performanceMetrics - Core Web Vitals and load timings
   */
  async diagnose({
    milestoneName,
    status,
...
2. Root Cause Analysis: If there are failures, errors, network 4xx/5xx codes, or Web Vitals degradation (e.g. LCP > 2.5s, slow TTFB, script execution bottlenecks), pinpoint the exact underlying technical causes (Shopify app scripts, unoptimized assets, API rejections, hydration issues, or DOM mutations).
3. Performance & Reliability Bottlenecks: Highlight top bottlenecks observed in the network and DOM lifecycle.
4. Actionable Engineering Fixes: Provide specific code, configuration, or infrastructure recommendations (e.g., resource preloads, deferring third-party analytics, caching strategies).
...
module.exports = new RootCauseAnalyst();
